...
interface AppPositionSelectProps extends Props {
	onChange: (option: unknown) => void;
}

export const AppPositionSelect = ({onChange, ...props}: AppPositionSelectProps) => {
	const dispatch = useAppDispatch();
	const [options, setOptions] = useState<OptionsOrGroups<unknown, GroupBase<unknown>>>([]);

	useEffect(() => {
		const promise = dispatch(globalAutocompleteThunk({search: "", index: "positions"}));
		promise.then((r) => {
	...
	return (
		<ReactSelect
			...
			onChange={onChange}
			options={options}
			searchOptions={searchOptions("positions")}

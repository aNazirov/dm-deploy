...
export const telemedicineReportService = {
	create: (body: IReportTelemedicineCreateParams, signal?: AbortSignal) => {
		return api
			.post<{telemedicineReport: TelemedicineReportModel}>(APIReportUrl.telemedicineReport, body, {signal})
	...
	get(params: IReportGetParams = {take: 20, skip: 0}, signal?: AbortSignal) {
		return api
			.get<{data: TelemedicineReportModel[]; count: number}>(APIReportUrl.telemedicineReport, {
				params: {params},
				signal,
			})
			.then((res) => {
	...
	getById(id: number, signal?: AbortSignal) {
		return api.get<TelemedicineReportModel>(`${APIReportUrl.telemedicineReport}/${id}`, {signal}).then((res) => {
	...
	update({id, body}: {id: number; body: Partial<IReportTelemedicineCreateParams>}, signal?: AbortSignal) {
		return api
			.patch<TelemedicineReportModel>(`${APIReportUrl.telemedicineReport}/${id}`, body, {signal})
			.then((res) => {
				return new TelemedicineReportModel(res.data);
			});
	},
	updateStatus({id, statusId}: {id: number; statusId: eReportStatusType}, signal?: AbortSignal) {
		return api
			.patch<TelemedicineReportModel>(`${APIReportUrl.telemedicineReport}/status/${id}`, {statusId}, {signal})
			.then((res) => {
	...
	delete(id: number, signal?: AbortSignal) {
		return api
			.delete<{message: string; status: number}>(`${APIReportUrl.telemedicineReport}/${id}`, {signal})
			.then((res) => {

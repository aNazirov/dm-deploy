...
export const trainingReportService = {
	create(body: IReportTrainingCreateParams, signal?: AbortSignal) {
		return api.post<{trainingReport: TrainingReportModel}>(APIReportUrl.trainingReport, body, {signal}).then((res) => {
	...
	get(params: IReportGetParams = {take: 20, skip: 0}, signal?: AbortSignal) {
		return api
			.get<{data: TrainingReportModel[]; count: number}>(APIReportUrl.trainingReport, {params: {params}, signal})
			.then((res) => {
				return {data: res.data.data.map((d) => new TrainingReportModel(d)), count: res.data.count};
			});
	},
	getById(id: number, signal?: AbortSignal) {
		return api.get<TrainingReportModel>(`${APIReportUrl.trainingReport}/${id}`, {signal}).then((res) => {
	...
	update({id, body}: {id: number; body: Partial<IReportTrainingCreateParams>}, signal?: AbortSignal) {
		return api.patch<TrainingReportModel>(`${APIReportUrl.trainingReport}/${id}`, body, {signal}).then((res) => {
			return new TrainingReportModel(res.data);
		});
	},
	updateStatus({id, statusId}: {id: number; statusId: eReportStatusType}, signal?: AbortSignal) {
		return api
			.patch<TrainingReportModel>(`${APIReportUrl.trainingReport}/status/${id}`, {statusId}, {signal})
			.then((res) => {
	...
	delete(id: number, signal?: AbortSignal) {
		return api
			.delete<{message: string; status: number}>(`${APIReportUrl.trainingReport}/${id}`, {signal})
			.then((res) => {

...
export const appealsReportService = {
	create(body: IReportAppealsCreateParams, signal?: AbortSignal) {
		return api.post<{appealsReport: AppealsReportModel}>(APIReportUrl.appealsReport, body, {signal}).then((res) => {
	...
	get(params: IReportGetParams = {take: 20, skip: 0}, signal?: AbortSignal) {
		return api
			.get<{data: AppealsReportModel[]; count: number}>(APIReportUrl.appealsReport, {params: {params}, signal})
			.then((res) => {
				return {data: res.data.data.map((d) => new AppealsReportModel(d)), count: res.data.count};
			});
	},
	getById(id: number, signal?: AbortSignal) {
		return api.get<AppealsReportModel>(`${APIReportUrl.appealsReport}/${id}`, {signal}).then((res) => {
	...
	update({id, body}: {id: number; body: Partial<IReportAppealsCreateParams>}, signal?: AbortSignal) {
		return api.patch<AppealsReportModel>(`${APIReportUrl.appealsReport}/${id}`, body, {signal}).then((res) => {
			return new AppealsReportModel(res.data);
		});
	},
	updateStatus({id, statusId}: {id: number; statusId: eReportStatusType}, signal?: AbortSignal) {
		return api
			.patch<AppealsReportModel>(`${APIReportUrl.appealsReport}/status/${id}`, {statusId}, {signal})
			.then((res) => {
	...
	delete(id: number, signal?: AbortSignal) {
		return api
			.delete<{message: string; status: number}>(`${APIReportUrl.appealsReport}/${id}`, {signal})
			.then((res) => {

...
export const dailyReportService = {
	...
	get(params: IReportGetParams = {take: 20, skip: 0}, signal?: AbortSignal) {
		return api
			.get<{data: DailyReportModel[]; count: number}>(APIReportUrl.daily, {params: {params}, signal})
			.then((res) => {
				return {data: res.data.data.map((d) => new DailyReportModel(d)), count: res.data.count};
			});
	},
	getById(id: number, signal?: AbortSignal) {
		return api.get<DailyReportModel>(`${APIReportUrl.daily}/${id}`, {signal}).then((res) => {
			return new DailyReportModel(res.data);
		});
	},
	update({id, body}: {id: number; body: Partial<IReportDailyCreateParams>}, signal?: AbortSignal) {
		return api.patch<DailyReportModel>(`${APIReportUrl.daily}/${id}`, body, {signal}).then((res) => {
			return new DailyReportModel(res.data);
		});
	},
	updateStatus({id, statusId}: {id: number; statusId: eReportStatusType}, signal?: AbortSignal) {
		return api.patch<DailyReportModel>(`${APIReportUrl.daily}/status/${id}`, {statusId}, {signal}).then((res) => {
	...
	delete(id: number, signal?: AbortSignal) {
		return api.delete<{message: string; status: number}>(`${APIReportUrl.daily}/${id}`, {signal}).then((res) => {

...
export const insuranceReportService = {
	create(body: IReportInsuranceCreateParams, signal?: AbortSignal) {
		return api
			.post<{insuranceReport: InsuranceReportModel}>(APIReportUrl.insuranceReport, body, {signal})
			.then((res) => {
	...
	get(params: IReportGetParams = {take: 20, skip: 0}, signal?: AbortSignal) {
		return api
			.get<{data: InsuranceReportModel[]; count: number}>(APIReportUrl.insuranceReport, {params: {params}, signal})
			.then((res) => {
				return {data: res.data.data.map((d) => new InsuranceReportModel(d)), count: res.data.count};
			});
	},
	getById(id: number, signal?: AbortSignal) {
		return api.get<InsuranceReportModel>(`${APIReportUrl.insuranceReport}/${id}`, {signal}).then((res) => {
	...
	update({id, body}: {id: number; body: Partial<IReportInsuranceCreateParams>}, signal?: AbortSignal) {
		return api.patch<InsuranceReportModel>(`${APIReportUrl.insuranceReport}/${id}`, body, {signal}).then((res) => {
			return new InsuranceReportModel(res.data);
		});
	},
	updateStatus({id, statusId}: {id: number; statusId: eReportStatusType}, signal?: AbortSignal) {
		return api
			.patch<InsuranceReportModel>(`${APIReportUrl.insuranceReport}/status/${id}`, {statusId}, {signal})
			.then((res) => {
	...
	delete(id: number, signal?: AbortSignal) {
		return api
			.delete<{message: string; status: number}>(`${APIReportUrl.insuranceReport}/${id}`, {signal})
			.then((res) => {

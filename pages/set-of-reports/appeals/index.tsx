...
import {ISetOfReportsAppeals, ISetOfReportsParams} from "../../../core/models";
import moment from "moment/moment";
import Moment from "react-moment";

const AppealsSetOfReportsPage = () => {
	const dispatch = useAppDispatch();

	const [results, setResults] = useState<{count: number; data: ISetOfReportsAppeals[]}>();
	...
	const renderRow = () => {
		return results?.data.map((r, i) => (
			<tr key={`${r.createdAt.toString()}-${i}`}>
				...
				<td>{r._sum.complaints}</td>
				<td>{r._sum.questions}</td>
			</tr>
		));
	};
	...
	return (
		<div className="pe-0 flex-col h-100">
			...
			<div className="pe-2.5">
				<AppSetOfReportsFilter exportUrl="appeals" onFilterSubmit={onFilterSubmit} />
			</div>
...
export default AppealsSetOfReportsPage;
